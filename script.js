// script.js completo atualizado conforme últimas modificações
...
function mostrarSugestoes() {
  let area = document.getElementById("sugestoes");
  if (!area) {
    area = document.createElement("div");
    area.id = "sugestoes";
    area.style.marginTop = "10px";
    area.style.fontSize = "1.1rem";
    document.body.insertBefore(area, document.getElementById("capturas"));
  }
  if (sugestoesIA.length === 0) {
    area.innerHTML = "";
  } else {
    area.innerHTML = `Sugestões da IA:<br>` + sugestoesIA.map((j, i) => `${i + 1}. ${j}`).join("<br>");
  }
}
