...
function criarTabuleiro() {
  ...
  for (let i = 0; i < 8; i++) {
    ...
    for (let j = 0; j < 8; j++) {
      ...
      if (peca) {
        const letrasBrancas = { p: "♙", r: "♖", n: "♘", b: "♗", q: "♕", k: "♔" };
        const letrasPretas = { p: "♟", r: "♜", n: "♞", b: "♝", q: "♛", k: "♚" };
        casa.textContent = peca.color === 'w' ? letrasBrancas[peca.type] : letrasPretas[peca.type];
        casa.dataset.peca = peca.color === 'w' ? "branca" : "preta";
        console.log(`Posição: ${posicao}, Cor da peça: ${peca.color}, data-peca: ${casa.dataset.peca}`);
